  ...
  return (
    <div className="p-6 glass rounded-2xl hover:border-gray-300 dark:hover:border-gray-600 transition-colors flex flex-col h-full relative">
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="text-xs font-bold text-black bg-green-100 border border-green-300 px-2.5 py-1 rounded-md shadow-sm">
          {student.match}% Match
        </div>
      </div>
      
      <p className="text-sm text-foreground/80 mb-4 line-clamp-2">"{student.description}"</p>
      
      <div className="flex flex-wrap gap-2 mb-5">
        {student.skills.map(skill => (
          <span 
            key={skill} 
            className="text-xs font-bold px-2.5 py-1 rounded-md bg-gray-100 border border-gray-300 text-black shadow-sm"
          >
            {skill}
          </span>
        ))}
      ...
      <div className="mt-auto">
        <Button 
          className="w-full bg-[#FF6B00] hover:bg-[#e66000] text-black font-bold shadow-md transition-colors"
          onClick={(e) => {
            e.stopPropagation()
