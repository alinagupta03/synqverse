...
export function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 bg-muted/80 flex items-start justify-center pt-[20vh] z-[100]" onClick={() => setIsOpen(false)}>
      <div className="bg-[#111] border border-border rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((action, i) => (
              <button
                ...
                className="w-full text-left px-4 py-3 rounded-xl hover:bg-gray-100 text-foreground text-sm transition-colors flex items-center justify-between group"
              >
                <span>{action.label}</span>
                <span className="text-xs text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">{action.section}</span>
              </button>
            ))
          )}
        </div>
