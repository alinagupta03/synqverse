...
export default function InnovationDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pt-10 px-6">
      <div className="max-w-5xl mx-auto space-y-12">
        <div className="relative p-10 glass rounded-3xl border border-border overflow-hidden">
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground mb-4">Innovation Accelerator</h1>
        ...
        <div>
          ...
          {submissions.length === 0 ? (
            <div className="p-12 text-center border border-border rounded-2xl bg-muted">
              <p className="text-lg font-medium text-foreground mb-2">No active applications</p>
              <p className="text-muted-foreground">Promote an idea to a project in your workspace, then submit it for support.</p>
            </div>
          ) : (
            <div className="grid gap-4">
              {submissions.map(sub => (
                <Link key={sub.id} href={`/innovation/${sub.id}`}>
                  <div className="glass p-6 rounded-2xl border border-border hover:border-gray-300 transition-all flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 group">
                    <div>
                      <p className="text-sm font-medium text-blue-400 mb-1">{sub.team.name}</p>
                      <h3 className="text-xl font-bold text-foreground group-hover:text-blue-300 transition-colors">{sub.project.idea.title}</h3>
                    </div>
