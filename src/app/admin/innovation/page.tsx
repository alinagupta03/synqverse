    ...
    <div className="p-6 md:p-12 max-w-7xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight text-foreground mb-2">Innovation Queue</h1>
          <p className="text-muted-foreground text-sm">Review, shortlist, and evaluate team project submissions.</p>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-white/5">
            {submissions.map(sub => (
              <tr key={sub.id} className="hover:bg-muted transition-colors">
                ...
                <td className="px-6 py-4 text-muted-foreground">{sub.team?.category || 'Other'}</td>
                <td className="px-6 py-4 text-muted-foreground truncate max-w-[200px]">{sub.expectedImpact}</td>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => handleAction(sub.id, 'SHORTLIST')} className="text-xs px-3 py-1 bg-purple-600 hover:bg-purple-700 text-foreground rounded transition-colors">Shortlist</button>
                    <button onClick={() => handleAction(sub.id, 'ACCEPT')} className="text-xs px-3 py-1 bg-green-600 hover:bg-green-700 text-foreground rounded transition-colors">Accept</button>
            ...
            {submissions.length === 0 && (
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-muted-foreground">No submissions in queue.</td>
              </tr>
            )}
          </tbody>
        </table>
