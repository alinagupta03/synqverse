    ...
    return (
      <div className="flex-1 overflow-y-auto bg-muted/40 p-6 md:p-12">
        <div className="max-w-4xl mx-auto space-y-8 text-center pt-24">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-foreground mb-2">No Active Project</h1>
          ...
          <Link href={`/workspace/${id}/ideas`}>
            <button className="bg-blue-600 hover:bg-blue-700 text-foreground px-6 py-3 rounded-lg font-medium transition-colors">Go to Idea Lab</button>
          </Link>
        </div>
      </div>
    )
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm text-muted-foreground">Workspace Core</span>
            </div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground mb-1">{project.idea.title}</h1>
          </div>
          
          {!submission ? (
            <Link href={`/innovation/new?workspaceId=${id}`}>
              <button className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-foreground px-6 py-2.5 rounded-lg text-sm font-medium transition-all shadow-lg shadow-blue-500/20 border border-border">
            ...
            <Link href={`/innovation/${submission.id}`}>
              <button className={`px-6 py-2.5 rounded-lg text-sm font-medium transition-all border ${
                ...
                'bg-gray-100 text-foreground border-border hover:bg-gray-200'
              }`}>
        ...
        <div className="glass p-6 rounded-2xl border border-border">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-semibold text-foreground">Project Stage</h3>
          </div>
            ...
            {STAGES.map((s, i) => {
              ...
              return (
                <div key={s} className="flex flex-col items-center gap-2">
                  <div className={`w-4 h-4 rounded-full border-2 ${current ? 'bg-muted border-blue-400 shadow-[0_0_10px_rgba(59,130,246,0.5)] scale-125' : active ? 'bg-blue-500 border-blue-500' : 'bg-muted border-gray-300'} transition-all`} />
                  <span className={`text-xs font-medium ${current ? 'text-blue-400' : active ? 'text-foreground' : 'text-muted-foreground'}`}>{s}</span>
                </div>
              )
            })}
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-6">
            <div className="glass p-6 rounded-2xl border border-border">
              <h3 className="font-semibold text-foreground mb-4">Core Definition</h3>
            ...
            <div className="glass p-6 rounded-2xl border border-border">
              <h3 className="font-semibold text-foreground mb-4">Target & Technology</h3>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Target Users</h4>
                  <p className="text-sm text-foreground/90">{project.targetUsers}</p>
                </div>
                <div>
                  <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Technology Stack</h4>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-foreground font-mono">{Math.round(project.idea.score)}/100</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-foreground font-mono">{project.idea.innovationLevel}/10</span>
                  </div>
